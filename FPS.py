'''
Original Author: Jean Machuca <correojean@gmail.com> @jeanmachuca
Adapted and Rewritten for CSCE462 Final Project TAMU Fall 2015 
    by Sean McLeod (mcleodseanm@gmail.com) and Jackie Sitzman
    Much of the code was not written befitting to our task, nor was this library complete/working; therefore much of the code was
     altered and there was a significant amount of code to debug/fix, along with several logic errors and minor formatting changes.
'''
...
def serial_ports():
    ...
    Many Serial functions not used.
    Most serial functions in file were inteded for use with windows and communication with USB ports.
    We retrofitted the file to work with the Pi's GPIO pins by opening a serial connection on /dev/ttyAMA0.
    '''
    if os.name == 'nt':
        # windows
...
if os.name == 'nt':
    ...
    DEVICE_NAME = '/dev/ttyAMA0'  #default device to use
    ...
    def ParseFromBytes(self,high,low):
        ...
        if high == 0x00:
            if low in self.errors.values():
                ...
                #print 'INDEX FOR ERROR MESSAGE: %s'% str(errorIndex)
...
def connect(device_name=None,baud=None,timeout=None,is_com=True):
    ...
    if isFingerPrintConnected(is_com):
        try:
            ...
            print '[Connect] Could not establish connection. %s' % (str(e))
            pass
    return _ser
...
class FPS_GT511C3(SerialCommander):
    ...
    def __init__(self,device_name=None,baud=None,timeout=None,is_com=True):
        ...
        if not self._serial is None:
            ...
            print '[FPS_GT511C3] Scanner Instance Failed to Create %s' % self._device_name
    ...
    def CheckEnrolled(self,ID):
        '''
             ...
             Parameter: 0-199
             Return: True if the ID number isn't enrolled, false if it is
        '''
        ...
        print rp.ACK
        del rp
        return not retval #not statement because logic is backwards for whatever reason
    ...
    def Identify1_N(self):
        ...
        delay(1)
        rp = self.GetResponse()
        #for x in rp.RawBytes
            #print(x)
        retval = rp.IntFromParameter()
        #print 'Return Value: %s'% str(retval)
        if retval > 200:
            retval = 200
        del rp
        del packetbytes
    ...
    def SendCommand(self, cmd,length): 
        if self._serial is not None:
            ...
            if self.UseSerialDebug:
                print '[SendCommand] Failed to send command. %s' % self._device_name
    ...
    def GetResponse(self):
        ...
        if self._serial is None:
            rp = Response_Packet()
            print '[GetResponse] No response received. %s' % self._device_name
        else:
            delay(interval)
            ...
            print 'GetResponse ACK: %s'% str(rp.ACK)
